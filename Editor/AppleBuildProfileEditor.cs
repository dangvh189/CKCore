...
#region Draw Build Profile Properties
            ...
            if (appleBuildProfile.AutomateInfoPlist)
            {
                ...
                if (string.IsNullOrEmpty(appleBuildProfile.MinimumOSVersion_iOS))
                {
                    appleBuildProfile.MinimumOSVersion_iOS = PlayerSettings.iOS.targetOSVersionString;
                }
                else
                {
                    PlayerSettings.iOS.targetOSVersionString = appleBuildProfile.MinimumOSVersion_iOS;
                }
                appleBuildProfile.MinimumOSVersion_iOS = EditorGUILayout.TextField(UIStrings.MinimumOSVersionFieldLabelText_iOS, appleBuildProfile.MinimumOSVersion_iOS);

                if (string.IsNullOrEmpty(appleBuildProfile.MinimumOSVersion_tvOS))
                {
                    appleBuildProfile.MinimumOSVersion_tvOS = PlayerSettings.tvOS.targetOSVersionString;
                }
                else
                {
                    PlayerSettings.tvOS.targetOSVersionString = appleBuildProfile.MinimumOSVersion_tvOS;
                }
                appleBuildProfile.MinimumOSVersion_tvOS = EditorGUILayout.TextField(UIStrings.MinimumOSVersionFieldLabelText_tvOS, appleBuildProfile.MinimumOSVersion_tvOS);

                if (string.IsNullOrEmpty(appleBuildProfile.MinimumOSVersion_macOS))
                {
                    appleBuildProfile.MinimumOSVersion_macOS = UIStrings.DefaultMinimumMacOSVersionText;
                }
                appleBuildProfile.MinimumOSVersion_macOS = EditorGUILayout.TextField(UIStrings.MinimumOSVersionFieldLabelText_macOS, appleBuildProfile.MinimumOSVersion_macOS);
            }
